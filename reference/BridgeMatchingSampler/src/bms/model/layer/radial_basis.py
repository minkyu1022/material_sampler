...
class ExponentialEnvelope(torch.nn.Module):
    """Exponential envelope function that ensures a smooth cutoff, as proposed in
    Unke, Chmiela, Gastegger, Schuett, Sauceda, Mueller 2021 (SpookyNet)."""
...
class BernsteinBasis(torch.nn.Module):
    """Bernstein polynomial basis, as proposed in Unke et al. 2021 (SpookyNet).
...
class RadialBasis(torch.nn.Module):
    """Radial basis with a smooth envelope.
    ...
    cutoff: float
        Cutoff distance in Angstrom.
    rbf: dict
        Basis function and its hyperparameters, e.g. ``{"name": "gaussian"}``.
    envelope: dict
        Envelope function, e.g. ``{"name": "polynomial", "exponent": 5}``.
    """
