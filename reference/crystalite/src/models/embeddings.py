...
class TemperatureEmbedder(nn.Module):
    """Embed a physical temperature while keeping NULL distinct from 0 K."""

    def __init__(self, d_model: int, reference_k: float = 750.0) -> None:
        super().__init__()
        if reference_k <= 0:
    ...
    def forward(self, temperature_k: torch.Tensor, present: torch.Tensor) -> torch.Tensor:
        if temperature_k.ndim != 1 or present.shape != temperature_k.shape:
            raise ValueError("temperature_k and present must both have shape (B,)")
        known = self.value((temperature_k.float() / self.reference_k).unsqueeze(-1))
        null = self.null.unsqueeze(0).expand_as(known)
        return torch.where(present.bool().unsqueeze(-1), known, null)
