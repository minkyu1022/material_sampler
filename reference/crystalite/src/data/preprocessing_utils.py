...
def segment_coo(src, index, ptr=None, dim_size=None, reduce="sum"):
    """Minimal segment_coo replacement supporting sum reduction."""
    if reduce != "sum":
        raise NotImplementedError("segment_coo currently only supports sum reduction")
    if ptr is not None:
        dim_size = ptr.numel() - 1
        index = torch.repeat_interleave(
            torch.arange(dim_size, device=src.device), ptr[1:] - ptr[:-1]
        )
    ...
    expanded_index = index.view(index.shape[0], *([1] * (src.dim() - 1)))
...
def segment_csr(src, indptr, reduce="sum"):
    """Minimal segment_csr replacement supporting sum reduction."""
    if reduce != "sum":
        raise NotImplementedError("segment_csr currently only supports sum reduction")

    out_size = indptr.numel() - 1
    out_shape = (out_size,) + tuple(src.shape[1:])
    out = src.new_zeros(out_shape)
    if src.numel() == 0:
        return out

    repeats = indptr[1:] - indptr[:-1]
    if src.shape[0] != repeats.sum():
        raise ValueError("segment_csr received inconsistent indptr and src shapes")

    index = torch.arange(out_size, device=src.device).repeat_interleave(repeats)
    index_view = index.view(index.shape[0], *([1] * (src.dim() - 1)))
    out.scatter_add_(0, index_view, src)
    return out
...
def _parse_float_array(text: str) -> np.ndarray:
    vals = [float(v) for v in _FLOAT_RE.findall(str(text))]
    return np.asarray(vals, dtype=np.float64)


def _parse_int_array(text: str) -> np.ndarray:
    vals = [int(v) for v in _INT_RE.findall(str(text))]
    return np.asarray(vals, dtype=np.int64)


def _material_id_from_row(row) -> str:
    mid = row.get("material_id", row.get("mp_id", row.get("id", None)))
    if mid is None or (isinstance(mid, float) and np.isnan(mid)):
        row_name = row.name if getattr(row, "name", None) is not None else "unknown"
        return f"row_{row_name}"
    return str(mid)


def _build_crystal_from_row(row, niggli=True, primitive=False):
    if "cif" in row.keys() and isinstance(row["cif"], str) and row["cif"].strip():
        crystal_str = row["cif"]
        crystal = build_crystal(crystal_str, niggli=niggli, primitive=primitive)
        return crystal, crystal_str, _material_id_from_row(row)

    required = ("positions", "cell", "atomic_numbers")
    if not all(k in row.keys() for k in required):
        raise KeyError(
            "Row must contain either 'cif' or all of "
            "'positions', 'cell', and 'atomic_numbers'."
        )

    atom_types = _parse_int_array(row["atomic_numbers"])
    if atom_types.size == 0:
        raise ValueError("Parsed zero atom types from 'atomic_numbers'.")

    frac_or_cart = _parse_float_array(row["positions"])
    if frac_or_cart.size != atom_types.size * 3:
        raise ValueError(
            f"positions has {frac_or_cart.size} values, expected {atom_types.size * 3}."
        )
    coords = frac_or_cart.reshape(atom_types.size, 3)

    cell_flat = _parse_float_array(row["cell"])
    if cell_flat.size != 9:
        raise ValueError(f"cell has {cell_flat.size} values, expected 9.")
    lattice = cell_flat.reshape(3, 3)

    crystal_raw = Structure(
    ...
    crystal_str = str(CifWriter(crystal_raw))
    crystal = build_crystal(crystal_str, niggli=niggli, primitive=primitive)
    return crystal, crystal_str, _material_id_from_row(row)
...
# https://github.com/FAIR-Chem/fairchem/blob/main/src/fairchem/core/common/utils.py#L566
...
def radius_graph_pbc(
    ...
    """TODO It seems like radius param is not even being used, and only max therehsold is used.
...
def process_one(
    ...
    crystal, crystal_str, material_id = _build_crystal_from_row(
    ...
    result_dict.update(
        {"mp_id": material_id, "cif": crystal_str, "graph_arrays": graph_arrays}
    )
