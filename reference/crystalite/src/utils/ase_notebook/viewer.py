...
from src.utils.ase_notebook.atom_info import create_info_lines
from src.utils.ase_notebook.atoms_convert import (
    convert_to_atoms,
    deserialize_atoms,
    serialize_atoms,
)
from src.utils.ase_notebook.backend.gui import AtomGui, AtomImages
from src.utils.ase_notebook.backend.svg import (
    create_axes_elements,
    create_svg_document,
    generate_svg_elements,
    svg_to_pil,
)
from src.utils.ase_notebook.backend.threejs import (
    RenderContainer,
    create_world_axes,
    generate_3js_render,
    make_basic_gui,
)
from src.utils.ase_notebook.color import lighten_webcolor
from src.utils.ase_notebook.configuration import ViewConfig
from src.utils.ase_notebook.data import load_data_file
from src.utils.ase_notebook.draw_utils import (
    compute_projection,
    get_rotation_matrix,
...
class AseView:
    ...
    def make_wandb_image(self, atoms, center_in_uc=False, repeat_uc=(1, 1, 1)):
        """Create an W&B Image of the atoms or structure (uses make_svg)."""
        import wandb

        svg = self.make_svg(atoms, center_in_uc=center_in_uc, repeat_uc=repeat_uc)
        return wandb.Image(svg_to_pil(svg))
    ...
    def launch_gui_subprocess(
        ...
        We encode all the data into a json object, then parse this to a console executable via
        stdin.

        :param test_init: wait for a x seconds, then test whether the process initialized without
            error.
        """
        atoms = convert_to_atoms(atoms)
        data_str = json.dumps(
