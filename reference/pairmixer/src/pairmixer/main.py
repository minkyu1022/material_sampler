...
from pairmixer.data.module.inferencev2 import Boltz2InferenceDataModule
from pairmixer.data.mol import load_canonicals
from pairmixer.data.msa.mmseqs2 import run_mmseqs2
from pairmixer.data.parse.a3m import parse_a3m
from pairmixer.data.parse.csv import parse_csv
from pairmixer.data.parse.fasta import parse_fasta
from pairmixer.data.parse.yaml import parse_yaml
from pairmixer.data.types import MSA, Manifest, Record
from pairmixer.data.write.writer import BoltzAffinityWriter, BoltzWriter
from pairmixer.model.models.boltz1 import Boltz1
from pairmixer.model.models.boltz2 import Boltz2
...
@rank_zero_only
def download_pairmixer(cache: Path) -> None:
    ...
    # Download model
    model = cache / "pairmixer.ckpt"
    if not model.exists():
        click.echo(
            f"Downloading the model weights to {model}. You may "
            "change the cache directory with the --cache flag."
        )
        for i, url in enumerate(PAIRMIXER_URL_WITH_FALLBACK):
            try:
                urllib.request.urlretrieve(url, str(model))  # noqa: S310
                break
            except Exception as e:  # noqa: BLE001
                if i == len(PAIRMIXER_URL_WITH_FALLBACK) - 1:
...
def check_inputs(data: Path) -> list[Path]:
    ...
    """
    click.echo(f"[{NODE_ID}/{NUM_NODES}] Checking input data.")

    # Check if data is a directory
    if data.is_dir():
        data: list[Path] = sorted(list(data.glob("*")))
        if NUM_NODES > 1:
            data = data[NODE_ID::NUM_NODES]  # noqa: E203
            print(f"Running on node {NODE_ID}/{NUM_NODES} with {len(data)} examples {data=}")
...
def filter_inputs_structure(
    ...
    if existing and not override:
        ...
        click.echo(msg)
        missing_msg = f"[{NODE_ID}/{NUM_NODES}] Found these existing items {existing}"
    ...
    try:
        ...
        for msa_idx, msa_id in enumerate(msas):
            ...
            if not msa_path.exists():
                new_msa_path = path.parent.parent / 'msa' / msa_path.name
                if new_msa_path.exists():
                    msa_path = new_msa_path
                else:
                    msg = f"MSA file {msa_path} and {new_msa_path} not found. "
...
def process_inputs(
    ...
    if records_dir.exists():
        ...
        if data:
            ...
            if NUM_NODES == 1:
                ...
                manifest_path = out_dir / "processed" / f"manifest_{NODE_ID}.json"
    ...
    click.echo(f"[{NODE_ID}/{NUM_NODES}] Processing {len(data)} inputs with {preprocessing_threads} threads.")

    if preprocessing_threads > 1 and len(data) > 1:
        with Pool(preprocessing_threads) as pool:
            record_paths = list(tqdm(pool.imap(process_input_partial, data), total=len(data)))
    else:
        record_paths = []
        for path in tqdm(data):
            record_paths.append(process_input_partial(path))
    record_paths = [p for p in record_paths if p is not None]

    # Load all records and write manifest
    # records = [Record.load(p) for p in records_dir.glob("*.json")]
    records = [Record.load(p) for p in record_paths]
...
@click.option(
    "--model",
    default="pairmixer",
    type=click.Choice(["boltz1", "boltz2", "pairmixer"]),
    help="The model to use for prediction. Default is pairmixer.",
)
@click.option(
    "--method",
    type=str,
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    model: Literal["boltz1", "boltz2", "pairmixer"] = "boltz2",
    ...
    if NUM_NODES > 1 and preprocessing_threads > 1:
        print(f"WARNING: NUM_NODES > 1 and preprocessing_threads > 1, never tried this before")
    ...
    elif model == "pairmixer":
        download_pairmixer(cache)
    else:
        msg = f"Model {model} not supported. Supported: boltz1, boltz2, pairmixer."
    ...
    trainer = Trainer(
        ...
        devices=devices,
        precision=32 if model in ["boltz1", "pairmixer"] else "bf16-mixed",
    )
    ...
    if filtered_manifest.records:
        ...
        if checkpoint is None:
            if model == "boltz2":
                ...
                checkpoint = cache / "pairmixer.ckpt"
            else:
