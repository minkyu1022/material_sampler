...
import pairmixer.model.layers.initialize as init
from pairmixer.data import const
from pairmixer.data.feature.symmetry import (
    minimum_lddt_symmetry_coords,
    minimum_symmetry_coords,
)
from pairmixer.model.loss.confidence import confidence_loss
from pairmixer.model.loss.distogram import distogram_loss
from pairmixer.model.loss.validation import (
    compute_pae_mae,
    compute_pde_mae,
    compute_plddt_mae,
    factored_lddt_loss,
...
from pairmixer.model.modules.confidence import ConfidenceModule
from pairmixer.model.modules.diffusion import AtomDiffusion
from pairmixer.model.modules.encoders import RelativePositionEncoder
from pairmixer.model.modules.trunk import (
    DistogramModule,
    InputEmbedder,
    MSAModule,
    PairformerModule,
)
from pairmixer.model.modules.pairmixer import PairmixerModule, PairmixerMSAModule
from pairmixer.model.modules.utils import ExponentialMovingAverage
from pairmixer.model.optim.scheduler import AlphaFoldLRScheduler
...
class Boltz1(LightningModule):
    ...
    def __init__(  # noqa: PLR0915, C901, PLR0912
        ...
        if not no_msa:
            if pairmixer:
                self.msa_module = PairmixerMSAModule(
                    token_z=token_z,
        ...
        if pairmixer:
            self.pairformer_module = PairmixerModule(token_z, **pairformer_args)
        else:
    ...
    def validation_step(self, batch: dict[str, Tensor], batch_idx: int):
        ...
        for m in const.out_types:
            # Skip types that don't exist in the computed dictionaries
