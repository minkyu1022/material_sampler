...
from pairmixer.data.module.training import BoltzTrainingDataModule, DataConfig
...
def train(raw_config: str, args: list[str]) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    if (out_dir / "models").exists() and not cfg.resume and not cfg.pretrained:
        model_files = list((out_dir / "models").glob("*.ckpt"))
        if model_files:
            latest_model = max(model_files, key=os.path.getmtime)
            print(f"Found existing model directory with checkpoints. Setting cfg.resume={latest_model}")
            cfg.resume = str(latest_model)
        else:
            print("Found empty models directory, starting training from scratch")

    if cfg.pretrained and not cfg.resume and not cfg.finetune:
    ...
    if cfg.validation_only:
        ...
        if cfg.finetune:
            if cfg.resume:
                print(f"WARNING: {cfg.finetune=} will be ignored when {cfg.resume=} is provided")
            else:
                print(f"Loading model weights from {cfg.finetune=}")
                checkpoint = torch.load(cfg.finetune, map_location='cpu', weights_only=False)
                if 'state_dict' in checkpoint:
                    checkpoint = checkpoint['state_dict']
                unexpected_keys, missing_keys = model_module.load_state_dict(checkpoint, strict=False)
                if unexpected_keys or missing_keys:
                    print(f"Loaded finetuned model weights without strict loading\n{unexpected_keys=}\n{missing_keys=}")
        trainer.fit(
            model_module,
            datamodule=data_module,
            ckpt_path=cfg.resume,
