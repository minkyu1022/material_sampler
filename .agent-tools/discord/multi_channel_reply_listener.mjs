...
async function discordFetchWithoutTranscriptAcknowledgement(input, init = {}) {
  ...
  if (init.method === "GET" && String(input).includes("discord.com/api/v10/channels/")) {
    const messages = await response.clone().json().catch(() => null);
    if (Array.isArray(messages)) {
      for (const message of messages) {
        const attachments = (message.attachments || [])
          .map((item) => `[attachment: ${item.filename}] ${item.url}`)
          .join("\n");
        if (attachments) message.content = [message.content, attachments].filter(Boolean).join("\n");
      }
